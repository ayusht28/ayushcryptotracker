...
import { Panel, SectionTitle, CoinAvatar, Badge, formatPrice, formatLarge, formatPct } from './ui';
...
export default function MarketTab({ prices, D }) {
  ...
  const filteredCoins = prices.filter(function(coin) {
  ...
  function closeDrawer() {
    setDrawerOpen(false);
    setTimeout(() => { setDrawer(null); setCoinDetail(null); setPriceHistory([]); }, 300);
  }
      ...
      {drawer && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: `1px solid ${D.border}` }}>
            ...
            <Badge
              label={formatPct(drawer.change_24h)}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${D.border}` }}>
              {['#', 'Coin', 'Price', '24H', 'Market Cap', 'Volume', ''].map(function(header, i) {
                return (
            ...
            {filteredCoins.map(function(coin, index) {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontSize: 13, fontWeight: 600, color: D.textBright, fontVariantNumeric: 'tabular-nums' }}>
